...
def carregar_agentes():
    conn = get_connection()
    cur = conn.cursor()
    rows = cur.execute("SELECT id, nome, telefone, pix, observacao, ativo FROM agentes ORDER BY nome").fetchall()
    conn.close()
    return [{"id": row["id"], "nome": row["nome"], "telefone": row["telefone"] or "", "pix": row["pix"] or "", "observacao": row["observacao"] or "", "ativo": row["ativo"]} for row in rows]
...
with st.form("form_novo_agente", clear_on_submit=True):
    ...
    if salvar:
        if not nome.strip():
            ...
            conn = get_connection()
            cur = conn.cursor()
            cur.execute("INSERT INTO agentes (nome, telefone, pix, observacao, ativo) VALUES (?, ?, ?, ?, ?)", (nome.strip(), telefone.strip(), pix.strip(), observacao.strip(), 1 if ativo else 0))
            conn.commit()
            conn.close()
...
if agentes:
    ...
    agente_id_sel = st.selectbox("Escolha um agente", options=ids_agentes, format_func=lambda aid: f'#{mapa_agentes[aid]["id"]} | {mapa_agentes[aid]["nome"]}')
        ...
        with st.form(f"form_editar_agente_{agente_sel['id']}"):
            ...
            if salvar_edicao:
                if not nome_edit.strip():
                    ...
                    conn = get_connection()
                    cur = conn.cursor()
                    cur.execute("UPDATE agentes SET nome = ?, telefone = ?, pix = ?, observacao = ?, ativo = ? WHERE id = ?", (nome_edit.strip(), telefone_edit.strip(), pix_edit.strip(), observacao_edit.strip(), 1 if ativo_edit else 0, agente_sel["id"]))
                    conn.commit()
                    conn.close()
...
agentes = carregar_agentes()
if agentes:
    dados = [{"ID": a["id"], "Nome": a["nome"], "Telefone": a["telefone"], "PIX": a["pix"], "Observação": a["observacao"], "Status": "Ativo" if a["ativo"] == 1 else "Inativo"} for a in agentes]
